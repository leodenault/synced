...
class BotAccessPageViewModel constructor(
  ...
  var rememberBotToken by mutableStateOf(false)
        ...
        onSuccess = {
          if (rememberBotToken) {
            settingsManager.commit(
              settingsManager.fetch().toBuilder().setBotToken(botToken).build()
            )
          }
          discordNavigator.navigateToServerSelector(it)
        },
        onFailure = {
          errorMessage = "Failed to authenticate with bot token. Is the token typed correctly?"
          isConnecting = false
        }
      )
    }
